...
const Attendance: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchGrades = async () => {
            try {
                const q = query(collection(db, 'grades'), where('schoolId', '==', currentUserData.schoolId));
                const snapshot = await getDocs(q);

                const gradesList = snapshot.docs.map(doc => doc.data().name).sort();
    ...
    useEffect(() => {
        ...
        const fetchSections = async () => {
            try {
                const q = query(collection(db, 'sections'), where('schoolId', '==', currentUserData.schoolId));
                const snapshot = await getDocs(q);

                const sectionsList = snapshot.docs.map(doc => doc.data().name).sort();
                                                    ...
                                                    <div className="flex justify-center gap-2">
                                                        {(['Present', 'Absent', 'Late', 'Excused'] as AttendanceStatus[]).map(status => (
                                                            <button
                                                                ...
                                                                className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${attendance[student.uid]?.status === status
